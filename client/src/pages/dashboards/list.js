...
const DashboardList = () => {
    ...
    const { data, isLoading, isError } = useList({
        resource: "users",metaData: { populate: ["businesses","educations"]}
    });
          ...
          render: (businesses) => (
            <ul>
              {businesses.map((business) => (
                <li key={business.id}>{business.businessrole}- {business.category}-{business.businessname}-{business.businessowner}</li>
              ))}
            </ul>
          ),
        },
